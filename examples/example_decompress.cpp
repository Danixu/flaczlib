...
int main() {
    ...
    while (!feof(in) || compressor.strm->avail_in || compressor.get_status() != FLACZLIB_RC_END_OF_STREAM) {
        if (!feof(in) && compressor.strm->avail_in < (sizeof(in_buffer) * 0.25)) {
            memmove(in_buffer, in_buffer + (sizeof(in_buffer) - compressor.strm->avail_in), compressor.strm->avail_in);

            size_t readed = fread(in_buffer + compressor.strm->avail_in, 1, sizeof(in_buffer) - compressor.strm->avail_in, in);
